...
namespace IBM.ICU.Charset
{
    ...
    public class TestBNF
    {
        ...
        static internal readonly String[] testRules = {
	            "$root = [ab]{3};",
        ...
        static internal String unicodeSetBNF = ""
                + "$root = $leaf | '[' $s $root2 $s ']' ;\r\n"
        ...
        public void Main()
        {
            ...
            {
                TestBNF.TestBNFMthd(testRules[i], null, 20);
            }

            TestBNF.TestBNFMthd(unicodeSetBNF, null, 20);
            // testParser();
        }
        ...
        /*
         * static void testParser() { try { Pick.Target target = new Pick.Target();
         * for (int i = 0; i < rules.length; ++i) { target.addRule(rules[i]); } }
        ...
        internal class Counts
        {
            ...
            internal Counts(int max)
            {
                counts = new int[max + 1];
            }

            internal void Inc(int index)
            {
                counts[index]++;
            }
            ...
            internal void Show()
            {
                System.Console.Out.WriteLine("Printing Counts");
                for (int i = 0; i < counts.Length; ++i)
                {
                    if (counts[i] == 0)
                        continue;
                    System.Console.Out.WriteLine(i + ": " + counts[i]);
                }
                System.Console.Out.WriteLine();
            }
        }
        ...
        public static void TestTokenizer()
        {
            ...
            String[] samples = { "a'b'c d #abc\r e", "'a '123 321", "\\\\", "a'b",
	                "a'", "abc def%?ghi", "%", "a", "\\ a", "a''''b" };
            for (int i = 0; i < samples.Length; ++i)
            {
                t.SetSource(samples[i]);
                System.Console.Out.WriteLine();
                System.Console.Out.WriteLine("Input: " + t.GetSource());
                int type = 0;
                while (type != IBM.ICU.Charset.Tokenizer.DONE)
                {
                    type = t.Next();
                    System.Console.Out.WriteLine(t.ToString(type, false));
                }
            }
        }

...
// --------------------------------------------------------------------------------------------------
 namespace IBM.ICU.Text {
	...
	/// <summary>
	/// A replacer that produces static text as its output. The text may contain
	/// transliterator stand-in characters that represent nested UnicodeReplacer
	/// objects, making it possible to encode a tree of replacers in a
	/// StringReplacer. A StringReplacer that contains such stand-ins is called a
	/// <em>complex</em> StringReplacer. A complex StringReplacer has a slower
	/// processing loop than a non-complex one.
	/// </summary>
	///
	internal class StringReplacer : UnicodeReplacer {
	
	    /// <summary>
	    /// Output text, possibly containing stand-in characters that represent
	    /// nested UnicodeReplacers.
	    /// </summary>
	    ///
	    ...
	    /// <summary>
	    /// A complex object contains nested replacers and requires more complex
	    /// processing. StringReplacers are initially assumed to be complex. If no
	    /// nested replacers are seen during processing, then isComplex is set to
	    /// false, and future replacements are short circuited for better
	    /// performance.
	    /// </summary>
	    ///
	    ...
	    /// <summary>
	    /// Object that translates stand-in characters in 'output' to UnicodeReplacer
	    /// objects.
	    /// </summary>
	    ///
	    ...
	    /// <param name="theCursorPos">cursor position that will be returned by the replace() method</param>
	    /// <param name="theData">transliterator context object that translates stand-incharacters to UnicodeReplacer objects</param>
	    public StringReplacer(String theOutput, int theCursorPos,
	    ...
	    public virtual String ToReplacerPattern(bool escapeUnprintable) {
	        ...
	        if (hasCursor && cursor < 0) {
	            while (cursor++ < 0) {
	                IBM.ICU.Impl.Utility.AppendToRule(rule, '@', true, escapeUnprintable,
	                        quoteBuf);
	            }
	        ...
	        for (int i = 0; i < output.Length; ++i) {
	            ...
	            UnicodeReplacer r = data.LookupReplacer(c);
	            if (r == null) {
	                IBM.ICU.Impl.Utility.AppendToRule(rule, c, false, escapeUnprintable,
	                        quoteBuf);
	            } else {
	                StringBuilder buf = new StringBuilder(" ");
	                buf.Append(r.ToReplacerPattern(escapeUnprintable));
	                buf.Append(' ');
	                IBM.ICU.Impl.Utility.AppendToRule(rule, buf.ToString(), true,
	        ...
	        IBM.ICU.Impl.Utility.AppendToRule(rule, -1, true, escapeUnprintable, quoteBuf);

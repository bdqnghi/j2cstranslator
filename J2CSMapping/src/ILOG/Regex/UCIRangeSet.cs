...
// --------------------------------------------------------------------------------------------------
 namespace ILOG.J2CsMapping.RegEx {
	...
	/// </summary>
	///
	internal class UCIRangeSet : LeafSet {
	
		private AbstractCharClass chars;
	
		private bool alt;
	
		public UCIRangeSet(AbstractCharClass cs, AbstractSet next) : base(next) {
			this.alt = false;
		...
		public override int Accepts(int strIndex, String testString) {
			return (chars.Contains(Char.ToLower(Char.ToUpper(testString[strIndex])))) ? 1 : -1;
		}

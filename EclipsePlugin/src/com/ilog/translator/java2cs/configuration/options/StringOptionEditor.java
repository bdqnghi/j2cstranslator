/**
 * 
 */
package com.ilog.translator.java2cs.configuration.options;

import org.eclipse.swt.SWT;
import org.eclipse.swt.events.ModifyEvent;
import org.eclipse.swt.events.ModifyListener;
import org.eclipse.swt.layout.GridData;
import org.eclipse.swt.layout.GridLayout;
import org.eclipse.swt.widgets.Composite;
import org.eclipse.swt.widgets.Control;
import org.eclipse.swt.widgets.Label;
import org.eclipse.swt.widgets.Text;

public class StringOptionEditor extends AbstractEditor<OptionImpl<String>> {

	private class OptionStringModifyListener implements ModifyListener {

		private final OptionImpl<String> option;

		public OptionStringModifyListener(OptionImpl<String> option) {
			this.option = option;
		}

		public void modifyText(ModifyEvent e) {
			option.setValue(text.getText());
		}
	}

	private Composite subComposite = null;
	private Text text = null;

	public Control createUIEditor(Composite parent, OptionImpl<String> option) {

		if (subComposite == null) {
			final GridLayout gridLayout = new GridLayout();
			gridLayout.numColumns = 2;
			gridLayout.makeColumnsEqualWidth = false;
			gridLayout.marginLeft = 0;
			gridLayout.marginWidth = 0;
			gridLayout.marginHeight = 0;

			final GridData gdSubComp = new GridData();
			gdSubComp.horizontalAlignment = SWT.FILL;

			subComposite = new Composite(parent, SWT.NULL);
			subComposite.setLayout(gridLayout);
			subComposite.setLayoutData(gdSubComp);

			final Label label = new Label(subComposite, SWT.LEFT);
			label.setText(getTextLabel(option.getName()));
			label.setToolTipText(getTooltip(option.getName()));

			final GridData gridData = new GridData();
			gridData.horizontalAlignment = SWT.FILL;
			gridData.grabExcessHorizontalSpace = true;
			gridData.verticalAlignment = SWT.CENTER;

			text = new Text(subComposite, SWT.BORDER);
			initializeValue(option);
			text.setToolTipText(getTooltip(option.getName()));
			text.addModifyListener(new OptionStringModifyListener(option));
			text.setLayoutData(gridData);
		}

		return subComposite;

	}

	public void initializeValue(OptionImpl<String> option) {
		if (text != null) {
			if (option.getValue() != null) {
				text.setText(option.getValue());
			} else {
				if (option.getDefaultValue() != null)
					text.setText(option.getDefaultValue());
			}
		}
	}
}
 ...
 * @internal
 */
public final class IlrCharStream implements CharStream {
	...
	// ------------------------------------------------------------
	// Constructors and reinitializations
	// ------------------------------------------------------------

	public IlrCharStream(Reader dstream) {
		this(dstream, 1, 1, 4096);
	}

	public IlrCharStream(Reader dstream, int buffersize) {
		this(dstream, 1, 1, buffersize);
	}

	public IlrCharStream(Reader dstream, int line, int column, int buffersize) {
		inputStream = dstream;
		this.line = line;
	...
	private final void UpdateLineColumn(char c) {
		...
		case '\n':
			...
			break;
		case '\t':
			// Corrected in JRules for tabulation number computing
			// column--;
			// column += (8 - (column & 07));
			break;
		default:
	...
	public final char readChar() throws IOException {
		...
		if (((buffer[bufpos] = c = ReadByte()) == '\\')) {
			...
			try {
				...
				buffer[bufpos] = c = (char) (IlrCharStream.hexval(c) << 12
						| IlrCharStream.hexval(ReadByte()) << 8
						| IlrCharStream.hexval(ReadByte()) << 4 | IlrCharStream
						.hexval(ReadByte()));

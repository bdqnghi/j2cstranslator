 ...
 */
public final class RenameJavaElementsDescriptor extends JavaRefactoringDescriptor {
	 ...
	 */
	private List<ILocalVariable> fJavaElements= null;
	 ...
	 */
	public RenameJavaElementsDescriptor(final String id) {
		super(id);
	...
	public RenameJavaElementsDescriptor(String id, String project, String description, String comment, Map arguments, int flags) {
		...
		if (getID().equals(IJavaRefactorings.RENAME_TYPE_PARAMETER)) {
			/*fJavaElements= JavaRefactoringDescriptorUtil.getJavaElement(fArguments, ATTRIBUTE_INPUT, getProject());
			String parameterName= JavaRefactoringDescriptorUtil.getString(fArguments, ATTRIBUTE_PARAMETER);
			if (fJavaElements instanceof IType) {
				fJavaElements= ((IType) fJavaElements).getTypeParameter(parameterName);
			}
			if (fJavaElements instanceof IMethod) {
				fJavaElements=((IMethod) fJavaElements).getTypeParameter(parameterName);
			}*/
		} else {
			//fJavaElements= JavaRefactoringDescriptorUtil.getJavaElement(fArguments, ATTRIBUTE_INPUT, getProject());
		}
		final int type = 0; //fJavaElements.getElementType();
	...
	protected void populateArgumentMap() {
		...
		if (getID().equals(IJavaRefactorings.RENAME_TYPE_PARAMETER)) {
			final ITypeParameter parameter= (ITypeParameter) fJavaElements;
			...
			// JavaRefactoringDescriptorUtil.setJavaElement(fArguments, ATTRIBUTE_INPUT, getProject(), fJavaElements);
		}
		final int type= 0; //fJavaElements.getElementType();
		if (type != IJavaElement.PACKAGE_FRAGMENT_ROOT)
	 ...
	 */
	public void setJavaElements(final List<ILocalVariable> elements) {
		Assert.isNotNull(elements);
		fJavaElements= elements;
	}

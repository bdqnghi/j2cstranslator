...
public class VariableVisibilityVisitor extends ASTChangeVisitor {
	...
	public boolean transform(IProgressMonitor pm, ASTNode cunit) {
		...
		if (localVariableToRename.size() == 0)
			return true;

		final Set<java.util.Map.Entry<ILocalVariable, String>> entries = localVariableToRename
				.entrySet();
	...
	public Change createChange(IProgressMonitor pm,
			List<ILocalVariable> locals, List<String> names)
			throws JavaModelException, CoreException {
		final RenameLocalVariablesProcessor processor = new RenameLocalVariablesProcessor(
				locals);
	...
	public class CollectLocalVariableToRename extends ASTVisitor {
		...
		public boolean visit(FieldDeclaration node) {
			...
			for (int i = 0; i < listFragments.size(); i++) {
				final VariableDeclarationFragment fragment = (VariableDeclarationFragment) listFragments
						.get(i);
		...
		/*
		 * TODO: Bug in JDT, in case of initialize, I can't find the JavaElement
		 * for a VariableDeclaration ... public boolean visit(Initializer node) {
		 * scope.push("s<init>"); return true; }
		 * 
		 * public void endVisit(Initializer node) { scope.pop(); }
		 */
		@Override
		...
		private void renameLocalVariable(VariableDeclaration /* Fragment */node,
				String newName) {
		...
		public class VariableScope {
		 ...
		 class MethodScope {
			...
			public String buildNewName(
					VariableDeclaration /* Fragment */fragment) {
				return fragment.getName().getIdentifier() + "_" + i++;
			}

			public void addInCurrentScope(
					VariableDeclaration /* Fragment */fragment) {
				if (!stack.isEmpty())
					stack.peek().localVariables.add(fragment.getName()
							.getIdentifier());
			}
			...
			public boolean nameClash(VariableDeclaration /* Fragment */fragment) {

				return (globaleVariables.contains(fragment.getName()
						.getIdentifier()))
						|| (!stack.isEmpty() && stack.peek().localVariables
								.contains(fragment.getName().getIdentifier()));
			}
		}
	}
}

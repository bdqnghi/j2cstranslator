...
public class ComputeTypeAndMethodDeclVisitor extends ASTRewriterVisitor {
	...
	public void endVisit(AssertStatement node) {
		...
		// TODO:
		final String classname = context.getModel()
				.getAssertReplacementClassName();
		final String newValueOfExpr = TranslationUtils.replaceByNewValue(expr,
				currentRewriter, fCu, context.getLogger());

		currentRewriter.replace(node, currentRewriter.createStringPlaceholder(
				classname + ".Assert(" + newValueOfExpr + ");", node
	...
	public void endVisit(SynchronizedStatement node) {
		...
		result.process(context, node, currentRewriter, nv.getRewriter(),
				description);
	...
	public void endVisit(SimpleType node) {
		if (node.getParent().getParent().getNodeType() != ASTNode.ARRAY_CREATION) {
			// Array creation already deal in
			// ComputeFieldAccessAndMethodInvocation (I hope ...)
			final INodeRewriter result = context.getMapper().mapSimpleType(fCu,
					node);

			final ASTNode cic = ASTNodes.getParent(node,
					ASTNode.CLASS_INSTANCE_CREATION);
			if (cic != null) {
				final String comment = TranslationUtils.getBeforeComments(cic,
						fCu, context);
				if (comment != null && comment.trim().equals("/* j2cs:done */")) {
					// do not proceed we already perform a modification here
					return;
				}
			}
	...
	public void endVisit(WildcardType node) {
		...
		if (bound == null) {
			if (isALocalVariableDeclarationType(node)
					|| isAMethodReturnType(node)) {
				final ASTNode replacement = currentRewriter
						.createStringPlaceholder("object", node.getNodeType());
		...
		if (TranslationUtils.isObjectType(typeB)) {
			currentRewriter.replace(node, ASTNode.copySubtree(node.getAST(),
					bound), description);
		}
		if ((ASTNodes.getParent(node, ASTNode.BLOCK) != null)
				|| isInAReturnType(node) || isAFieldDeclaration(node)) {
			final Object nv = currentRewriter.getEventStore().getNewValue(
					bound.getParent(), bound.getLocationInParent());
			if (nv != null)
				currentRewriter.replace(node, (ASTNode) nv, description);
			else
				currentRewriter.replace(node, currentRewriter
						.createCopyTarget(bound), description);
		}
	}
	...
	public void endVisit(ParameterizedType node) {
		final INodeRewriter result = context.getMapper().mapParameterizedType(
				fCu, node);

		final ASTNode cic = ASTNodes.getParent(node,
				ASTNode.CLASS_INSTANCE_CREATION);
		if (cic != null) {
			final String comment = TranslationUtils.getBeforeComments(cic, fCu,
					context);
			if (comment != null && comment.trim().equals("/* j2cs:done */")) {
				// do not proceed we already perform a modification here
				return;
		...
		// replace Inner<T> by Enclosing<T>.Inner<T> when needed
		...
		if (enclosingType != null // && enclosingType.isMember()
				&& (tArgs.length > 0 || tPars.length > 0)) {
			final ITypeBinding innerParametrizedType = node.resolveBinding();
			if (innerParametrizedType != null
					&& innerParametrizedType.isMember()) {
				ITypeBinding declaringType = innerParametrizedType
						.getDeclaringClass();
				if (declaringType != null) {
					if (declaringType.isRawType()) {
						declaringType = declaringType.getErasure();
						// here we have <E> (enclosing type parameter name)
						final String declName = replaceEnclosingTypeParam(
								declaringType, innerParametrizedType);
						if (declName != null) {
							final String replacement = "/* insert_here:"
									+ declName + ". */"
									+ innerParametrizedType.getName();
							final ASTNode replNode = currentRewriter
									.createStringPlaceholder(replacement, node
											.getNodeType());
							currentRewriter
									.replace(node, replNode, description);
						}
					} else {
						declaringType = declaringType.getErasure();
						// here we have <E> (enclosing type parameter name)
	...
	public boolean visit(QualifiedName node) {
		...
		if ((binding != null)
				&& (node.resolveBinding().getKind() == IBinding.TYPE)) {
			...
			result = context.getMapper().mapPackageAccess(node,
					binding.getJavaElement().getJavaProject().getProject());
	...
	public void endVisit(TypeDeclaration node) {
		if (implementsSerializable(node.resolveBinding())) {
			if (!node.resolveBinding().isInterface())
				context.addSerializable(node);
			removeSerializable(node);
		}

		final INodeRewriter result = context.getMapper().mapTypeDeclaration(
				node.resolveBinding(), context.getHandlerFromDoc(node, false));
		applyNodeRewriter(node, result, description);
	}
	...
	public void endVisit(MethodDeclaration node) {
		if (Modifier.isSynchronized(node.getModifiers())) {
			// TODO : bad bad bad idea !!!!!
			context.addSynchronized(node);
		}

		final INodeRewriter result = context.getMapper().mapMethodDeclaration(
				fCu.getElementName(), context.getSignatureFromDoc(node, false),
				context.getHandlerFromDoc(node, true), false, false, true);
	...
	private String replaceEnclosingTypeParam(ITypeBinding enclosingType,
			ITypeBinding nestedarametrizedType) {
		// enclosingType containt type param (such as <E>)
		// and parametrisedType contain renamed type param (such as <E_1>)
		final ITypeBinding[] typeParamForEnclosing = enclosingType
				.getTypeParameters();
		final ITypeBinding[] typeParamForNested = nestedarametrizedType
				.getTypeArguments();
		final List<String> typeParamNameForNested = getNames(typeParamForNested);
		if (typeParamForEnclosing.length == 0)
			return null;
		String fName = "<";
		for (int i = 0; i < typeParamForEnclosing.length; i++) {
			final ITypeBinding currentTB = typeParamForEnclosing[i];
			if (typeParamNameForNested.contains(currentTB.getName() + "_1"))
				fName += currentTB.getName() + "_1";
			else
				fName += currentTB.getName();
			if (i < typeParamForEnclosing.length - 1)
				fName += ",";
		}
		fName += ">";
	...
	private void removeSerializable(TypeDeclaration node) {
		...
		for (final Type type : interfaces) {
			...
			if (typeBinding.getQualifiedName().equals(
	...
	private boolean isInAReturnType(ASTNode node) {
		if (node == null || node.getParent() == null
			...
			return false;
		if (node.getLocationInParent().equals(
				MethodDeclaration.RETURN_TYPE2_PROPERTY)) {
			final MethodDeclaration methDecl = (MethodDeclaration) node
					.getParent();
			if (TranslationUtils.containsTag(methDecl,
					TranslationModelUtil.GENERIC_METHOD_WITHCONSTRAINT_TAG)
					|| TranslationUtils.containsTag(methDecl,
							TranslationModelUtil.GENERIC_WILDCARD_TAG))
				return false;
			return true;
		}

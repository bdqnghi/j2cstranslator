...
public class TranslateResourcesAction extends ActionDelegate implements
	 ...
	 */
	public TranslateResourcesAction() {
		super();
	}
	...
	public void migrate(ICompilationUnit compilationUnit) {
		...
		translateResourceFiles(javaProject);
	}

	public void translateResourceFiles(final IJavaProject javaProject) {
		try {
			...
			globalOptions.getLogger().logInfo(
					"--- Translating resources for project '"
				...
				public void run(IProgressMonitor monitor) {
					try {
						...
						configuration.init(options.getGlobalOptions()
								.getConfigurationFileName(), false);
						ITranslator translator = new Translator(javaProject,
								configuration);
						translator.copyResources(monitor);
						globalOptions.getLogger().logInfo("--- done.");

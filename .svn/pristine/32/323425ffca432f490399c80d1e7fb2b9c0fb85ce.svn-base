					...
					if (searchFor(insert_C, size, buffer, i)) {
						...
						for (int j = i + insert_lC; !end; j++) {
							if (j + " */".length() < size
								...
								final String insertexpr = buffer.getText(
										start_comment + insert_lC, end_comment
												- start_comment - insert_lC);
								edits.add(new ReplaceEdit(start_comment,
										end_comment - start_comment + 3,
										insertexpr));
							}
						}
						...
						for (int j = i + remove_lC; !end; j++) {
							if (j + " */".length() < size
								...
								// Remove all until we found ".class"
								// Insert what we extract from comment
								final String removeCpt = buffer.getText(
										start_comment + insert_lC, end_comment
												- start_comment - insert_lC);
								final int cpt = Integer.parseInt(removeCpt);
								final int length = end_comment - start_comment
										+ 3 + cpt;
								edits.add(new ReplaceEdit(start_comment,
										length, ""));
							}

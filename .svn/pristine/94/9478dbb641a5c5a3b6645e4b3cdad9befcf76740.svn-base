...
public class AutomaticInferGenericArgumentVisitor extends ASTRewriterVisitor {
	...
	public boolean visit(MethodInvocation node) {
		...
		// check if the method has an implicit generic argument
		// List<T> mthd() for example
		if (node.typeArguments().size() == 0 && isValidMethod(mBinding)) {
			...
			for (final ITypeBinding tParam : typeParams) {
				lr.insertLast(currentRewriter.createStringPlaceholder(tParam
						.getQualifiedName(), ASTNode.SIMPLE_TYPE), description);
			}
		}
		return true;
	}

...
public class PrepareExtensionMethodsForEnum2 extends ASTRewriterVisitor {
	...
	public void endVisit(ThisExpression node) {
		...
		//We are in the new method created to be transformed in extension method
		context.getLogger().logInfo(
				"[Custom info] Found this in enumeration "
				+ ((EnumDeclaration) enumParent).getName()
				+ " for method :"
				+ ((MethodDeclaration) methodParent).getName());
		SimpleName newExpression = currentRewriter.getAST()
		.newSimpleName(
				PrepareExtensionMethodsForEnum1.PARAMETER_NAME);

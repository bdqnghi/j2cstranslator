...
// --------------------------------------------------------------------------------------------------
 namespace IBM.ICU.Util {
	...
	/// can return clones of internal objects, since those are safe. <h2>Background</h2>
	/// <p>
	...
	/// <li>Immutable wrappers. For example, you can put a collection in an immutable
	/// wrapper.</li>
	/// <li>Always-Immutable objects. Java uses this approach, with a few variations.
	/// Examples:
	/// <ol>
	...
	/// <li>Always-Immutable Objects are safe, but usage can require excessive object
	/// creation.</li>
	...
	/// <p>
	/// The <c>Freezable</c> model supplements these choices by giving you the
	/// ability to build up an object by calling various methods, then when it is in
	/// a final state, you can <i>make</i> it immutable. Once immutable, an object
	/// cannot <i>ever </i>be modified, and is completely thread-safe: that is,
	/// multiple threads can have references to it without any synchronization. If
	/// someone needs a mutable version of an object, they can use
	...
	/// </pre>
	/// <p>
	/// Add a <c>cloneAsThawed()</c> method following the normal pattern for
	/// <c>clone()</c>, except that <c>frozen=false</c> in the new clone.
	/// </p>
	/// <p>
	...
	/// <p>
	/// Any subclass of a <c>Freezable</c> will just use its superclass's
	/// flagging field. It must override <c>freeze()</c> and
	/// <c>cloneAsThawed()</c> to call the superclass, but normally does not
	/// override <c>isFrozen()</c>. It must then just pay attention to its own
	...
	/// <p>
	/// Internal fields are called <i>safe</i> if they are either <c>frozen</c>
	/// or immutable (such as String or primitives). If you've never allowed internal
	/// access to these, then you are all done. For example, converting UnicodeSet to
	/// be <c>Freezable</c> is just accomplished with the above steps. But
	/// remember that you <i><b>have</b></i> allowed access to unsafe internals if
	/// you have any code like the following, in a getter, setter, or constructor:
	/// </p>
	/// <pre>
	/// Collection getStuff() {
	/// return stuff;
	...
	/// work in the <code>
	/// freeze()</code> function. Just make all of your internal fields
	/// frozen, and set the frozen flag. Any subsequent getter/setter will work
	...
	/// </pre>
	/// <p>
	/// If the field is a <c>Collection</c> or <c>Map</c>, then to make
	/// it frozen you have two choices. If you have never allowed access to the
	...
	/// </pre>
	/// <p>
	/// If you have <i>ever</i> allowed access, then do a <c>clone()</c> before
	/// wrapping it.
	/// </p>
	/// <pre>
	...
	/// </pre>
	/// <p>
	/// If a collection <i>(or any other container of objects)</i> itself can contain
	/// mutable objects, then for a safe clone you need to recurse through it to make
	/// the entire collection immutable. The recursing code should pick the most
	/// specific collection available, to avoid the necessity of later downcasing.
	/// </p>
	/// <blockquote>
	/// <p>
	...
	///
	/// @stable ICU 3.8
	public interface Freezable /*: ICloneable*/ {
	    /// <summary>

...
namespace ILOG.J2CsMapping.Text
{
	...
	/// <summary>
	/// <p>
	/// <c>CollationElementIterator</c> is created by a
	/// <c>RuleBasedCollator</c> to iterate through a string. The return result
	...
	public sealed class CollationElementIterator {
	    ...
	    /// <c>next()</c> and <c>previous()</c> when the end or the
	    ...
	    private IBM.ICU.Text.CollationElementIterator icuIterator;
	
	    internal CollationElementIterator(IBM.ICU.Text.CollationElementIterator iterator) {
	    ...
	    /// element as the last element, returns <c>1</c>.
	    /// </summary>
	    ///
	    /// <param name="order">a collation element that has been previously obtained from acall to either the <see cref="M:System.CollationElementIterator.Next"/> or <see cref="M:System.CollationElementIterator.Previous"/>method.</param>
	    ...
	    /// next collation element. This value could be any of: <ui> <li>The index of
	    /// the first character in the source string that matches the value of the
	    /// next collation element. (This means that if setOffset(offset) sets the
	    /// index in the middle of a contraction, getOffset() returns the index of
	    /// the first character in the contraction, which may not be equal to the
	    /// original offset that was set. Hence calling getOffset() immediately after
	    /// setOffset(offset) does not guarantee that the original offset set will be
	    /// returned.)</li> <li>If normalization is on, the index of the immediate
	    /// subsequent character, or composite character with the first character,
	    /// having a combining class of 0.</li> <li>The length of the source string,
	    /// if iteration has reached the end.</li> <ui>
	    /// </summary>
	    ///
	    /// <returns>The position of the collation element in the source string that
	    /// will be returned in the next invocation of the 
	    /// <see cref="M:System.CollationElementIterator.Next"/>
	    /// method.</returns>
	    ...
	    /// </summary>
	    ///
	    /// <returns>the next collation element or <c>NULLORDER</c> if the end
	    /// of the iteration has been reached.</returns>
	    public int Next() {
	    ...
	    /// </summary>
	    ///
	    /// <returns>the previous collation element, or <c>NULLORDER</c> when
	    /// the start of the iteration has been reached.</returns>
	    public int Previous() {
	    ...
	    public static int PrimaryOrder(int order) {
	        return IBM.ICU.Text.CollationElementIterator.PrimaryOrder(order);
	    }
	
	    /// <summary>
	    /// Repositions the cursor to point at the first element of the current
	    /// string. The next call to <c>next()</c> or <c>previous()</c>
	    /// will return the first and last collation element in the string,
	    /// respectively.
	    /// <p>
	    /// If the <c>RuleBasedCollator</c> used by this iterator has had its
	    /// attributes changed, calling <c>reset()</c> will reinitialize the
	    ...
	    ///
	    /// <param name="order"></param>
	    /// <returns>the 8 bit secondary order of the element</returns>
	    public static short SecondaryOrder(int order) {
	        return (short) IBM.ICU.Text.CollationElementIterator
	    ...
	    /// After this call completes, an invocation of the <see cref="M:System.CollationElementIterator.Next"/> method
	    /// will return this collation element.
	    /// <p>
	    /// If <c>newOffset</c> corresponds to a character which is part of a
	    ...
	    /// made to <c>getOffset()</c> may not return the same value set by
	    /// this method.
	    /// </p>
	    /// <p>
	    ...
	    /// </summary>
	    ///
	    /// <param name="newOffset">the character offset into the original source string to set.Note that this is not an offset into the correspondingsequence of collation elements.</param>
	    ...
	    ///
	    /// <param name="order"></param>
	    /// <returns>the 8 bits tertiary order of the element</returns>

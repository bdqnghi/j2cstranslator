...
public class RenameFieldVisitor extends ASTRewriterVisitor {
	...
	public void endVisit(VariableDeclarationFragment node) {
		...
		if (result != null && result instanceof ElementRewriter)
			((ElementRewriter) result).setModifyModifiers(true);
		applyNodeRewriter(node, result, description);
	}

	@Override
	public boolean visit(FieldDeclaration node) {
		final FieldRewriter result = (FieldRewriter) context.getMapper()

...
public class ReferenceAnalyzer2 extends MoveStaticMemberAnalyzer2 {
	...
	public ReferenceAnalyzer2(CompilationUnitRewrite cuRewrite,
			IBinding[] members, ITypeBinding target, ITypeBinding source) {
	...
	public boolean visit(FieldDeclaration node) {
		...
		final VariableDeclarationFragment singleFragment = (VariableDeclarationFragment) node
				.fragments().get(0);
	...
	public boolean visit(SimpleName node) {
		if (!node.isDeclaration() && isMovedMember(node.resolveBinding())
				&& !isProcessed(node)) {
	...
	public boolean visit(QualifiedName node) {
		if (isMovedMember(node.resolveBinding())) {
			if (node.getParent() instanceof ImportDeclaration) {
				...
				if (typeBinding != null) {
					fCuRewrite.getImportRewrite().removeImport(
							typeBinding.getQualifiedName());
				}
				final String imp = fCuRewrite.getImportRewrite().addImport(
						fTarget.getQualifiedName() + '.'

...
public class MoveClassInMethodToInnerVisitor extends ASTRewriterVisitor {
	...
	public boolean visit(TypeDeclaration node) {
		if (ASTNodes.getParent(node, ASTNode.METHOD_DECLARATION) != null) {
			final TypeDeclaration newType = (TypeDeclaration) currentRewriter
					.createMoveTarget(node);
			currentRewriter.remove(node, description);
			final TypeDeclaration enclosingTypeOfMethod = (TypeDeclaration) ASTNodes
					.getParent(node, ASTNode.TYPE_DECLARATION);
			currentRewriter.getListRewrite(enclosingTypeOfMethod,
					enclosingTypeOfMethod.getBodyDeclarationsProperty())
					.insertLast(newType, description);
		}
		//
		// Search for auto property if requested
		//

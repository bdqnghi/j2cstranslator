...
/**
 * Helper class to adjust the visibilities of members with respect to a
 * reference element.
 * 
 * @since 3.1
 */
public final class MemberVisibilityAdjustor2 {
	 ...
	 * @since 3.2
	 */
	public static final GroupCategorySet SET_VISIBILITY_ADJUSTMENTS = new GroupCategorySet(
			new GroupCategory(
	...
	public static class IncomingMemberVisibilityAdjustment implements
			IVisibilityAdjustment2 {
		 ...
		 * 
		 * @param member
		 *            the member which is adjusted
		 * @param keyword
		 *            the keyword representing the adjusted visibility
		 * @param status
		 *            the associated status, or <code>null</code>
		 */
		public IncomingMemberVisibilityAdjustment(final IMember member,
				final ModifierKeyword keyword, final RefactoringStatus status) {
			Assert.isNotNull(member);
			Assert.isTrue(!(member instanceof IInitializer));
			Assert.isTrue(MemberVisibilityAdjustor2
					.isVisibilityKeyword(keyword));
			fMember = member;
			fKeyword = keyword;
		 ...
		 * Does the visibility adjustment need rewriting?
		 * 
		 * @return <code>true</code> if it needs rewriting, <code>false</code>
		 *         otherwise
		 */
		 ...
		 * @param root
		 *            the root of the AST used in the rewrite
		 * @param group
		 *            the text edit group description to use, or
		 *            <code>null</code>
		 * @param status
		 *            the refactoring status, or <code>null</code>
		 * @throws JavaModelException
		 *             if an error occurs
		 */
		@SuppressWarnings("unchecked")
		protected final void rewriteVisibility(
				final MemberVisibilityAdjustor2 adjustor,
				final ASTRewrite rewrite, final CompilationUnit root,
			...
			final int visibility = fKeyword != null ? fKeyword.toFlagValue()
					: Modifier.NONE;
			if ((fMember instanceof IField)
					&& !Flags.isEnum(fMember.getFlags())) {
				final VariableDeclarationFragment fragment = ASTNodeSearchUtil
						.getFieldDeclarationFragmentNode((IField) fMember, root);
				final FieldDeclaration declaration = (FieldDeclaration) fragment
						.getParent();
				if (declaration.fragments().size() == 1) {
					ModifierRewrite.create(rewrite, declaration).setVisibility(
							visibility, group);
				} else {
					final VariableDeclarationFragment newFragment = rewrite
							.getAST().newVariableDeclarationFragment();
					newFragment.setName((SimpleName) rewrite
							.createCopyTarget(fragment.getName()));
					final FieldDeclaration newDeclaration = rewrite.getAST()
							.newFieldDeclaration(newFragment);
					newDeclaration.setType((Type) rewrite
							.createCopyTarget(declaration.getType()));
					IExtendedModifier extended = null;
					for (final Iterator iterator = declaration.modifiers()
							.iterator(); iterator.hasNext();) {
						extended = (IExtendedModifier) iterator.next();
						if (extended.isModifier()) {
							final Modifier modifier = (Modifier) extended;
							final int flag = modifier.getKeyword()
									.toFlagValue();
						...
						newDeclaration.modifiers().add(
								rewrite.createCopyTarget((ASTNode) extended));
					}
					ModifierRewrite.create(rewrite, newDeclaration)
							.setVisibility(visibility, group);
					final AbstractTypeDeclaration type = (AbstractTypeDeclaration) declaration
							.getParent();
					rewrite.getListRewrite(type,
							type.getBodyDeclarationsProperty()).insertAfter(
							newDeclaration, declaration, null);
					final ListRewrite list = rewrite.getListRewrite(
							declaration, FieldDeclaration.FRAGMENTS_PROPERTY);
				...
				final BodyDeclaration declaration = ASTNodeSearchUtil
						.getBodyDeclarationNode(fMember, root);
				if (declaration != null) {
					ModifierRewrite.create(rewrite, declaration).setVisibility(
							visibility, group);
					if (status != null) {
		...
		/*
		 * @see org.eclipse.jdt.internal.corext.refactoring.structure.MemberVisibilityAdjustor.IVisibilityAdjustment#rewriteVisibility(org.eclipse.jdt.internal.corext.refactoring.structure.MemberVisibilityAdjustor,
		 *      org.eclipse.core.runtime.IProgressMonitor)
		 */
		public void rewriteVisibility(final MemberVisibilityAdjustor2 adjustor,
			...
			try {
				...
				monitor
						.setTaskName(RefactoringCoreMessages.MemberVisibilityAdjustor_adjusting);
				if (fNeedsRewriting) {
					if ((adjustor.fRewrite != null) && (adjustor.fRoot != null)) {
						this.rewriteVisibility(adjustor, adjustor.fRewrite,
								adjustor.fRoot, null, fRefactoringStatus);
					} else {
						final CompilationUnitRewrite rewrite = adjustor
								.getCompilationUnitRewrite(fMember
										.getCompilationUnit());
						this
								.rewriteVisibility(
										adjustor,
												...
												.createCategorizedGroupDescription(
														Messages
																.format(
																		...
																		MemberVisibilityAdjustor2
																				.getLabel(getKeyword())),
														MemberVisibilityAdjustor2.SET_VISIBILITY_ADJUSTMENTS),
										fRefactoringStatus);
					}
		 ...
		 * 
		 * @param rewriting
		 *            <code>true</code> if it needs rewriting,
	...
	public interface IVisibilityAdjustment2 {
		 ...
		 *             if an error occurs
		 */
		public void rewriteVisibility(MemberVisibilityAdjustor2 adjustor,
				IProgressMonitor monitor) throws JavaModelException;
	}

	/** Description of an outgoing member visibility adjustment */
	public static class OutgoingMemberVisibilityAdjustment extends
		 ...
		 */
		public OutgoingMemberVisibilityAdjustment(final IMember member,
				final ModifierKeyword keyword, final RefactoringStatus status) {
	 ...
	 * 
	 * @param keyword
	 *            the keyword to get the label for, or <code>null</code> for
	 *            default visibility
	 * @return the label for the keyword
	 */
	public static String getLabel(final ModifierKeyword keyword) {
		Assert.isTrue(MemberVisibilityAdjustor2.isVisibilityKeyword(keyword));
		if (keyword == null) {
	...
	public static String getMessage(final IMember member) {
		Assert.isTrue((member instanceof IType) || (member instanceof IMethod)
	...
	/**
	 * Do the specified modifiers represent a lower visibility than the required
	 * threshold?
	 * 
	 * @param modifiers
	 *            the modifiers to test
	 * @param threshold
	 *            the visibility threshold to compare with
	 * @return <code>true</code> if the visibility is lower than required,
	 *         <code>false</code> otherwise
	 */
	public static boolean hasLowerVisibility(final int modifiers,
			final int threshold) {
	...
	/**
	 * Does the specified modifier keyword represent a lower visibility than the
	 * required threshold?
	 * 
	 * @param keyword
	 *            the visibility keyword to test, or <code>null</code> for
	 *            default visibility
	 * @param threshold
	 *            the visibility threshold keyword to compare with, or
	 *            <code>null</code> to compare with default visibility
	 * @return <code>true</code> if the visibility is lower than required,
	 *         <code>false</code> otherwise
	 */
	public static boolean hasLowerVisibility(final ModifierKeyword keyword,
			final ModifierKeyword threshold) {
		Assert.isTrue(MemberVisibilityAdjustor2.isVisibilityKeyword(keyword));
		Assert.isTrue(MemberVisibilityAdjustor2.isVisibilityKeyword(threshold));
		return MemberVisibilityAdjustor2.hasLowerVisibility(
	 ...
	 * @return <code>true</code> if it is a refactoring status severity,
	 *         <code>false</code> otherwise
	 */
	 ...
	 * 
	 * @param keyword
	 *            the keyword to test, or <code>null</code>
	 * @return <code>true</code> if it is a visibility keyword,
	 *         <code>false</code> otherwise
	 */
	 ...
	 * @param modifier
	 *            the keyword to test
	 * @return <code>true</code> if it is a visibility modifier,
	 *         <code>false</code> otherwise
	 */
	private static boolean isVisibilityModifier(final int modifier) {
		return (modifier == Modifier.NONE) || (modifier == Modifier.PUBLIC)
	 ...
	 *            the member to test
	 * @param threshold
	 *            the visibility threshold to test for
	 * @param adjustments
	 *            the map of members to visibility adjustments
	 * @return <code>true</code> if the member needs further adjustment,
	 *         <code>false</code> otherwise
	 */
	@SuppressWarnings("unchecked")
	public static boolean needsVisibilityAdjustments(final IMember member,
			final int threshold, final Map adjustments) {
		Assert.isNotNull(member);
		Assert
				.isTrue(MemberVisibilityAdjustor2
						.isVisibilityModifier(threshold));
		Assert.isNotNull(adjustments);
		final IncomingMemberVisibilityAdjustment adjustment = (IncomingMemberVisibilityAdjustment) adjustments
				.get(member);
		if (adjustment != null) {
			final ModifierKeyword keyword = adjustment.getKeyword();
			return MemberVisibilityAdjustor2.hasLowerVisibility(
					keyword == null ? Modifier.NONE : keyword.toFlagValue(),
					threshold);
		}
		return true;
	}
	 ...
	 *            the member to test
	 * @param threshold
	 *            the visibility threshold to test for, or <code>null</code>
	...
	@SuppressWarnings("unchecked")
	public static boolean needsVisibilityAdjustments(final IMember member,
			final ModifierKeyword threshold, final Map adjustments) {
		Assert.isNotNull(member);
		Assert.isNotNull(adjustments);
		final IncomingMemberVisibilityAdjustment adjustment = (IncomingMemberVisibilityAdjustment) adjustments
				.get(member);
		if (adjustment != null) {
			return MemberVisibilityAdjustor2.hasLowerVisibility(adjustment
					.getKeyword(), threshold);
		}
		return true;
	}
	...
	/**
	 * The AST rewrite to use for reference visibility adjustments, or
	 * <code>null</code> to use a compilation unit rewrite
	 */
	...
	/**
	 * The root node of the AST rewrite for reference visibility adjustments, or
	 * <code>null</code> to use a compilation unit rewrite
	 */
	 ...
	 * 
	 * @param referencing
	 *            the referencing element used to compute the visibility
	 * @param referenced
	 *            the referenced member which causes the visibility changes
	 */
	public MemberVisibilityAdjustor2(final IJavaElement referencing,
			final IMember referenced) {
		Assert.isTrue(!(referenced instanceof IInitializer));
		Assert.isTrue((referencing instanceof ICompilationUnit)
				|| (referencing instanceof IType)
				|| (referencing instanceof IPackageFragment));
		fScope = RefactoringScopeFactory.createReferencedScope(
	 ...
	 * 
	 * @param element
	 *            the "source" point from which to calculate the visibility
	 * @param referencedMovedElement
	 *            the moved element which may be adjusted in visibility
	 * @param monitor
	 *            the progress monitor to use
	 * @throws JavaModelException
	 *             if the visibility adjustment could not be computed
	 */
	@SuppressWarnings("unchecked")
	private void adjustIncomingVisibility(final IJavaElement element,
			IMember referencedMovedElement, final IProgressMonitor monitor)
			throws JavaModelException {
		final ModifierKeyword threshold = getVisibilityThreshold(element,
				referencedMovedElement, monitor);
		final int flags = referencedMovedElement.getFlags();
		final IType declaring = referencedMovedElement.getDeclaringType();
		if (declaring.isInterface()) {
			return;
		}
		if (MemberVisibilityAdjustor2.hasLowerVisibility(flags,
				threshold == null ? Modifier.NONE : threshold.toFlagValue())
											...
											.createStatus(
													...
													Messages
															.format(
																	...
																	new String[] {
																			MemberVisibilityAdjustor2
																					.getLabel(referencedMovedElement),
																			MemberVisibilityAdjustor2
																					.getLabel(threshold) }),
													JavaStatusContext
															.create(referencedMovedElement),
													null,
	 ...
	 */
	private void adjustMemberVisibility(final IMember member,
			final IProgressMonitor monitor) throws JavaModelException {

		if (member instanceof IType) {
			// recursively check accessibility of member type's members
			final IJavaElement[] typeMembers = ((IType) member).getChildren();
			for (final IJavaElement element : typeMembers) {
				if (!(element instanceof IInitializer)) {
					adjustMemberVisibility((IMember) element, monitor);
				}
			}
		}

		if ((member.equals(fReferenced))
				|| (Modifier.isPublic(member.getFlags()))) {
			return;
		}

		final SearchResultGroup[] references = findReferences(member, monitor);
		for (final SearchResultGroup element : references) {
			final SearchMatch[] searchResults = element.getSearchResults();
			for (final SearchMatch element0 : searchResults) {
				...
				if ((fAdjustments.get(member) == null)
					...
					this.adjustIncomingVisibility(fReferencing, member,
	 ...
	 * @param element
	 *            the element
	 * @return <code>true</code> if it is inside, <code>false</code>
	 *         otherwise
	 */
	 ...
	 */
	private SearchResultGroup[] findReferences(final IMember member,
			final IProgressMonitor monitor) throws JavaModelException {
		final RefactoringSearchEngine2 engine = new RefactoringSearchEngine2(
				SearchPattern.createPattern(member,
	 ...
	 * 
	 * @param groups
	 *            the search result groups representing the references
	 * @param monitor
	 *            the progress monitor to use
	 * @throws JavaModelException
	 *             if the java elements could not be accessed
	 */
	private void adjustIncomingVisibility(final SearchResultGroup[] groups,
		...
		try {
			...
			monitor
					.setTaskName(RefactoringCoreMessages.MemberVisibilityAdjustor_checking);
			SearchMatch[] matches = null;
			boolean adjusted = false;
			for (final SearchResultGroup element0 : groups) {
				matches = element0.getSearchResults();
				for (final SearchMatch element1 : matches) {
					final Object element = element1.getElement();
					if ((element instanceof IMember)
							&& !isInsideMovedMember((IMember) element)) {
						// found one reference which is not inside the moved
						// element => adjust visibility of the moved element
						this.adjustIncomingVisibility(fReferencing,
								fReferenced, monitor);
	...
	/**
	 * Adjusts the visibility of the referenced field found in a compilation
	 * unit.
	 * 
	 * @param field
	 *            the referenced field to adjust
	 * @param threshold
	 *            the visibility threshold, or <code>null</code> for default
	 *            visibility
	 * @throws JavaModelException
	 *             if an error occurs
	 */
	private void adjustOutgoingVisibility(final IField field,
			final ModifierKeyword threshold) throws JavaModelException {
		Assert.isTrue(!field.isBinary() && !field.isReadOnly());
		// bug 100555 (moving inner class to top level class; taking private
		// fields with you)
		final IType declaring = field.getDeclaringType();
		if ((declaring != null) && declaring.equals(fReferenced)) {
			return;
		}
		if (MemberVisibilityAdjustor2.hasLowerVisibility(field.getFlags(),
				MemberVisibilityAdjustor2.keywordToVisibility(threshold))
	 ...
	 * 
	 * @param member
	 *            the member where to adjust the visibility
	 * @param threshold
	 *            the visibility keyword representing the required visibility,
	 *            or <code>null</code> for default visibility
	 * @param template
	 ...
	 */
	@SuppressWarnings("unchecked")
	private void adjustOutgoingVisibility(final IMember member,
			final ModifierKeyword threshold, final String template)
			throws JavaModelException {
		Assert.isTrue(!member.isBinary() && !member.isReadOnly());
		boolean adjust = true;
		final IType declaring = member.getDeclaringType();
		if ((declaring != null)
				&& (JavaModelUtil.isInterfaceOrAnnotation(declaring) || declaring
						.equals(fReferenced))) {
			adjust = false;
		}
		if (adjust
				&& MemberVisibilityAdjustor2.hasLowerVisibility(member
						.getFlags(), MemberVisibilityAdjustor2
						.keywordToVisibility(threshold))
	...
	/**
	 * Adjusts the visibilities of the referenced element from the search match
	 * found in a compilation unit.
	 * 
	 * @param match
	 *            the search match representing the element declaration
	 * @param monitor
	 *            the progress monitor to use
	 * @throws JavaModelException
	 *             if the visibility could not be determined
	 */
	private void adjustOutgoingVisibility(final SearchMatch match,
			final IProgressMonitor monitor) throws JavaModelException {
		final Object element = match.getElement();
		if (element instanceof IMember) {
			final IMember member = (IMember) element;
			if (!member.isBinary() && !member.isReadOnly()
					&& !isInsideMovedMember(member)) {
				adjustOutgoingVisibilityChain(member, monitor);
			}
		}
	}

	private void adjustOutgoingVisibilityChain(final IMember member,
			final IProgressMonitor monitor) throws JavaModelException {

		try {
			if (!Modifier.isPublic(member.getFlags())) {
				final ModifierKeyword threshold = computeOutgoingVisibilityThreshold(
						fReferencing, member, monitor);
			...
			if (member.getDeclaringType() != null) {
				adjustOutgoingVisibilityChain(member.getDeclaringType(),
						monitor);
			}
		} catch (final Exception e) {
			// TODO : Juste ignore the case we can't do it !
		}
	}

	/**
	 * Adjusts the visibilities of the outgoing references from the member
	 * represented by the specified search result groups.
	 * 
	 * @param groups
	 *            the search result groups representing the references
	 * @param monitor
	 *            the progress monitor to us
	 * @throws JavaModelException
	 *             if the visibility could not be determined
	 */
		...
		try {
			...
			for (final SearchResultGroup element0 : groups) {
				...
				if (element instanceof ICompilationUnit) {
					...
					for (final SearchMatch element1 : matches) {
						this.adjustOutgoingVisibility(element1,
								new SubProgressMonitor(monitor, 1));
					}
				} // else if (element != null)
				// fStatus.merge(RefactoringStatus.createStatus(fFailureSeverity,
				// RefactoringCoreMessages.getFormattedString("MemberVisibilityAdjustor.binary.outgoing.project",
				// new String[] { element.getJavaProject().getElementName(),
				// getLabel(fReferenced)}), null, null,
				// RefactoringStatusEntry.NO_CODE, null)); //$NON-NLS-1$
				// else if (group.getResource() != null)
				// fStatus.merge(RefactoringStatus.createStatus(fFailureSeverity,
				// RefactoringCoreMessages.getFormattedString("MemberVisibilityAdjustor.binary.outgoing.resource",
				// new String[] { group.getResource().getName(),
				// getLabel(fReferenced)}), null, null,
				// RefactoringStatusEntry.NO_CODE, null)); //$NON-NLS-1$
	 ...
	 *             if an error occurs during search
	 */
	public final void adjustVisibility(final IProgressMonitor monitor)
			throws JavaModelException {
		try {
			...
			monitor
					.setTaskName(RefactoringCoreMessages.MemberVisibilityAdjustor_checking);
			final RefactoringSearchEngine2 engine = new RefactoringSearchEngine2(
					SearchPattern.createPattern(fReferenced,
							IJavaSearchConstants.REFERENCES,
							SearchUtils.GENERICS_AGNOSTIC_MATCH_RULE));
			...
			if (fIncoming) {
				...
				engine.searchPattern(new SubProgressMonitor(monitor, 1));
				this.adjustIncomingVisibility((SearchResultGroup[]) engine
						.getResults(), new SubProgressMonitor(monitor, 1));
				engine.clearResults();
				...
				if (fReferenced instanceof IType) {
					...
					adjustMemberVisibility(type, new SubProgressMonitor(
							monitor, 1));
				}
			}
			if (fOutgoing) {
				/*
				 * Search for the types, fields, and methods which are
				 * called/acted upon inside the referenced element (the one to
				 * be moved) and assure that they are also visible from within
				 * the referencing element (the destination type (or package if
				 * move to new type)).
				 */
				engine.searchReferencedTypes(fReferenced,
						new SubProgressMonitor(monitor, 1,
								SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
				engine.searchReferencedFields(fReferenced,
						new SubProgressMonitor(monitor, 1,
								SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
				engine.searchReferencedMethods(fReferenced,
						new SubProgressMonitor(monitor, 1,
								SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
				this.adjustOutgoingVisibility((SearchResultGroup[]) engine
						.getResults(), new SubProgressMonitor(monitor, 1));
			}
		} finally {
			monitor.done();
	 ...
	 * @param monitor
	 *            the progress monitor to use
	 * @return the visibility keyword corresponding to the threshold, or
	 *         <code>null</code> for default visibility
	 * @throws JavaModelException
	 *             if the java elements could not be accessed
	 */
	public ModifierKeyword getVisibilityThreshold(
		...
		try {
			...
			case IJavaElement.TYPE: {
				...
				final ICompilationUnit referencedUnit = typeReferenced
						.getCompilationUnit();
				switch (referencingType) {
				case IJavaElement.TYPE: {
					keyword = thresholdTypeToType((IType) referencing,
							typeReferenced, monitor);
					break;
				}
				...
				case IJavaElement.METHOD: {
					...
					if (typeReferenced.equals(member.getDeclaringType())) {
						keyword = ModifierKeyword.PRIVATE_KEYWORD;
							...
							&& referencedUnit.equals(member
									.getCompilationUnit())) {
						keyword = ModifierKeyword.PRIVATE_KEYWORD;
					} else if (typeReferenced.getPackageFragment().equals(
							member.getDeclaringType().getPackageFragment())) {
						keyword = null;
					}
					break;
				}
			...
			case IJavaElement.FIELD: {
				...
				final ICompilationUnit referencedUnit = fieldReferenced
						.getCompilationUnit();
				switch (referencingType) {
				case IJavaElement.TYPE: {
					keyword = thresholdTypeToField((IType) referencing,
							fieldReferenced, monitor);
					break;
				}
				case IJavaElement.FIELD:
				case IJavaElement.METHOD: {
					final IMember member = (IMember) referencing;
					if (fieldReferenced.getDeclaringType().equals(
						...
						keyword = ModifierKeyword.PRIVATE_KEYWORD;
							...
							&& referencedUnit.equals(member
									.getCompilationUnit())) {
						keyword = ModifierKeyword.PRIVATE_KEYWORD;
					} else if (fieldReferenced.getDeclaringType()
							.getPackageFragment().equals(
									member.getDeclaringType()
											.getPackageFragment())) {
						keyword = null;
					}
				...
				case IJavaElement.PACKAGE_FRAGMENT: {
					final IPackageFragment fragment = (IPackageFragment) referencing;
					if (fieldReferenced.getDeclaringType().getPackageFragment()
							.equals(fragment)) {
						keyword = null;
					}
					break;
			...
			case IJavaElement.METHOD: {
				...
				final ICompilationUnit referencedUnit = methodReferenced
						.getCompilationUnit();
				switch (referencingType) {
				case IJavaElement.TYPE: {
					keyword = thresholdTypeToMethod((IType) referencing,
							methodReferenced, monitor);
					break;
				}
				case IJavaElement.FIELD:
				case IJavaElement.METHOD: {
					final IMember member = (IMember) referencing;
					if (methodReferenced.getDeclaringType().equals(
							member.getDeclaringType())) {
						keyword = ModifierKeyword.PRIVATE_KEYWORD;
					} else if ((referencedUnit != null)
							&& referencedUnit.equals(member
									.getCompilationUnit())) {
						keyword = ModifierKeyword.PRIVATE_KEYWORD;
					} else if (methodReferenced.getDeclaringType()
							.getPackageFragment().equals(
									member.getDeclaringType()
											.getPackageFragment())) {
						keyword = null;
				...
				case IJavaElement.PACKAGE_FRAGMENT: {
					final IPackageFragment fragment = (IPackageFragment) referencing;
					if (methodReferenced.getDeclaringType()
							.getPackageFragment().equals(fragment)) {
						keyword = null;
					}
					break;
				}
	...
	private ModifierKeyword computeOutgoingVisibilityThreshold(
		...
		Assert.isTrue((referencing instanceof ICompilationUnit)
				|| (referencing instanceof IType)
				|| (referencing instanceof IPackageFragment));
		Assert.isTrue((referenced instanceof IType)
				...
				case IJavaElement.COMPILATION_UNIT: {
					...
					if ((referencedUnit != null) && referencedUnit.equals(unit)) {
						keyword = ModifierKeyword.PRIVATE_KEYWORD;
							...
							&& referencedUnit.getParent().equals(
									unit.getParent())) {
						keyword = null;
					}
					break;
	 ...
	 * 
	 * @param unit
	 *            the compilation unit to get the rewrite for
	 * @return the rewrite for the compilation unit
	 */
	private CompilationUnitRewrite getCompilationUnitRewrite(
			final ICompilationUnit unit) {
		CompilationUnitRewrite rewrite = (CompilationUnitRewrite) fRewrites
				.get(unit);
		if (rewrite == null) {
			if (fOwner == null) {
	 ...
	 * @throws JavaModelException
	 *             if the type hierarchy could not be created
	 */
	private ITypeHierarchy getTypeHierarchy(final IType type,
			final IProgressMonitor monitor) throws JavaModelException {
		...
		try {
			...
			try {
				...
				if (hierarchy == null) {
					if (fOwner == null) {
						hierarchy = type
								.newSupertypeHierarchy(new SubProgressMonitor(
										monitor,
										1,
										SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
					} else {
	 ...
	 *         <code>false</code> otherwise
	 */
	private boolean needsVisibilityAdjustment(final IMember member,
			final ModifierKeyword threshold) {
		Assert.isNotNull(member);
		return MemberVisibilityAdjustor2.needsVisibilityAdjustments(member,
				threshold, fAdjustments);
	}
	 ...
	 * 
	 * @param unit
	 *            the compilation unit to rewrite the adjustments
	 * @param monitor
	 *            the progress monitor to use
	 * @throws JavaModelException
	 *             if an error occurs during search
	 */
	@SuppressWarnings("unchecked")
	public final void rewriteVisibility(final ICompilationUnit unit,
			final IProgressMonitor monitor) throws JavaModelException {
		try {
			...
			monitor
					.setTaskName(RefactoringCoreMessages.MemberVisibilityAdjustor_adjusting);
			IMember member = null;
			IVisibilityAdjustment2 adjustment = null;
			for (final Iterator iterator = fAdjustments.keySet().iterator(); iterator
					.hasNext();) {
				member = (IMember) iterator.next();
				if (unit.equals(member.getCompilationUnit())
						&& fAdjustments.get(member) instanceof IVisibilityAdjustment2) {
					adjustment = (IVisibilityAdjustment2) fAdjustments
							.get(member);
					if (adjustment != null) {
						adjustment.rewriteVisibility(this,
								new SubProgressMonitor(monitor, 1));
					}
				}
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final void rewriteVisibility(final IProgressMonitor monitor)
			throws JavaModelException {
		try {
			monitor.beginTask("", fAdjustments.keySet().size()); //$NON-NLS-1$
			monitor
			...
			for (final Iterator iterator = fAdjustments.keySet().iterator(); iterator
					.hasNext();) {
				member = (IMember) iterator.next();
				adjustment = (IVisibilityAdjustment2) fAdjustments.get(member);
				if (adjustment != null) {
					adjustment.rewriteVisibility(this, new SubProgressMonitor(
							monitor, 1));
				}
			}
		} finally {
	...
	/**
	 * Sets the existing visibility adjustments to be taken into account
	 * (element type: Map <IMember, IVisibilityAdjustment>).
	 * <p>
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is to take no existing adjustments into account.
	 * 
	 * @param adjustments
	 ...
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is a status with value {@link RefactoringStatus#ERROR}.
	 * 
	 * @param severity
	 *            the severity of failure messages
	 */
	public final void setFailureSeverity(final int severity) {
		Assert.isTrue(MemberVisibilityAdjustor2.isStatusSeverity(severity));
	}
	 ...
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is to adjust incoming references.
	 * 
	 * @param incoming
	 *            <code>true</code> if incoming references should be adjusted,
	 *            <code>false</code> otherwise
	 */
	 ...
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is to adjust outgoing references.
	 * 
	 * @param outgoing
	 *            <code>true</code> if outgoing references should be adjusted,
	 *            <code>false</code> otherwise
	 */
	 ...
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is to use a compilation unit rewrite.
	 * 
	 * @param rewrite
	...
	/**
	 * Sets the compilation unit rewrites used by this adjustor (element type:
	 * Map <ICompilationUnit, CompilationUnitRewrite>).
	 * <p>
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is to use no existing rewrites.
	 * 
	 * @param rewrites
	 *            the map of compilation units to compilation unit rewrites to
	 *            set
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is the whole workspace as scope.
	 * 
	 * @param scope
	 ...
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is a fresh status with status {@link RefactoringStatus#OK}.
	 * 
	 * @param status
	 *            the refactoring status to set
	 */
	 ...
	 * This method must be called before calling
	 * {@link MemberVisibilityAdjustor#adjustVisibility(IProgressMonitor)}. The
	 * default is a status with value {@link RefactoringStatus#WARNING}.
	 * 
	 * @param severity
	 *            the severity of visibility messages
	 */
	 ...
	 *             if the java elements could not be accessed
	 */
	private ModifierKeyword thresholdTypeToField(final IType referencing,
			final IField referenced, final IProgressMonitor monitor)
			throws JavaModelException {
		ModifierKeyword keyword = ModifierKeyword.PUBLIC_KEYWORD;
		...
		if (referenced.getDeclaringType().equals(referencing)) {
			...
			final ITypeHierarchy hierarchy = getTypeHierarchy(referencing,
					new SubProgressMonitor(monitor, 1));
			final IType[] types = hierarchy.getSupertypes(referencing);
			IType superType = null;
			for (final IType element : types) {
				superType = element;
		...
		if ((referencedUnit != null) && referencedUnit.equals(typeUnit)) {
			keyword = ModifierKeyword.PRIVATE_KEYWORD;
		} else if ((referencedUnit != null) && (typeUnit != null)
				&& referencedUnit.getParent().equals(typeUnit.getParent())) {
			keyword = null;
		}
		return keyword;
	}
	 ...
	 *             if the java elements could not be accessed
	 */
	private ModifierKeyword thresholdTypeToMethod(final IType referencing,
			final IMethod referenced, final IProgressMonitor monitor)
			throws JavaModelException {
		final ICompilationUnit referencedUnit = referenced.getCompilationUnit();
		ModifierKeyword keyword = ModifierKeyword.PUBLIC_KEYWORD;
		if (referenced.getDeclaringType().equals(referencing)) {
	 ...
	 *             if the java elements could not be accessed
	 */
	private ModifierKeyword thresholdTypeToType(final IType referencing,
			final IType referenced, final IProgressMonitor monitor)
			throws JavaModelException {
		ModifierKeyword keyword = ModifierKeyword.PUBLIC_KEYWORD;
		final ICompilationUnit referencedUnit = referenced.getCompilationUnit();

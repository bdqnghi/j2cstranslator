...
public class MethodInfo extends MemberInfo {
	...
	public String resolveType(String typeName) {
		try {
			final String[][] resolved = method.getDeclaringType().resolveType(
					typeName);
	...
	public String toFile() {
		...
		if (targetMethods != null && targetMethods.size() > 0) {
			...
			for(final TargetMethod targetMethod : targetMethods.values()) {
				...
				if (targetMethod.isTranslated())
					res += " remove = true;";
				if (targetMethod.isRenamed())
					res += " renamed = true;";
		...
		for (int i = 0; i < method.getParameterTypes().length; i++) {
			...
			try {
				...
				if (!typename.contains(".")) {
					String[][] resTN = method.getDeclaringType().resolveType(
							typename);
					if (resTN.length == 1)
						typename = Signature.toQualifiedName(resTN[0]);
				}			
			} catch (Exception e) {
	...
	// <method signature="" name="" isExcluded="true|false">
	//    <target .../>
	// </method>
	// or
	// <constructor signature="" isExcluded="true|false">
	//    <target .../>
	// </method>
	public void toXML(StringBuilder res, String tabValue) {
		res.append(Constants.THREETAB + "<!--                               -->\n");
		res.append(Constants.THREETAB + "<!-- method " + name.getValue()
				+ Utils.xmlify(getSignature()) + " -->\n");
		res.append(Constants.THREETAB + "<!--                               -->\n");
		if (isConstructor()) {
			res.append(Constants.THREETAB + "<constructor signature=\""
					+ Utils.xmlify(getSignature()) + "\"");
		} else {
			res.append(Constants.THREETAB + "<method signature=\""
					+ Utils.xmlify(getSignature()) + "\" name=\"" + name.getValue()
					+ "\"");
		}
		...
		res.append(">\n");
		if (targetMethods != null && targetMethods.size() > 0) {
			for(TargetMethod m : targetMethods.values())
				m.toXML(res, tabValue);
		}
		if (isConstructor())
			res.append(Constants.THREETAB + "</constructor>");
		else
			res.append(Constants.THREETAB + "</method>");
	}
	...
	private static boolean needCapitalization(ClassInfo cinfo) {
		...
		if (cinfo.getTarget(targetFramework) != null) {
			if (cinfo.getTarget(targetFramework).getMemberMappingBehavior() != null) {
				neededByClassInfo = cinfo.getTarget(targetFramework).getMemberMappingBehavior().equals(
						MethodMappingPolicy.CAPITALIZED);
			} 
		}
		if (neededByClassInfo)
			return neededByClassInfo;
		else {
			...
			if (pInfo.getTarget(targetFramework) != null) {
				if (pInfo.getTarget(targetFramework).getMemberMappingBehavior() != null) {
					neededByPackageInfo = pInfo.getTarget(targetFramework).getMemberMappingBehavior().equals(
							MethodMappingPolicy.CAPITALIZED);
				} 
			}
			if (neededByPackageInfo)
				return neededByPackageInfo;
		}
		return false;
	}
				...
				if (elemName.equals("target")) {
					...
					if (!pckElement.getAttribute("autoProperty").isEmpty()) {
						...
						if (mName.startsWith("get")) {
							 ...
							 pName = Utils.capitalize(mName); // TODO: is it the expected behavior ?
							 tProperty.getPropertyGetOption().setValue(pName);
							 tProperty.setKind(ProperyKind.READ);
							 tProperty.setRewriter(new PropertyRewriter(pName, ProperyKind.READ));
						} else {
	...
	private boolean isProperty(Element node) {
		return !node.getAttribute("propertyGet").isEmpty()
				|| !node.getAttribute("propertySet").isEmpty();
	}
	...
	private boolean isIndexer(Element node) {
		return !node.getAttribute("indexerGet").isEmpty()
				|| !node.getAttribute("indexerSet").isEmpty();
	}
